...
- **VLA-layer TTT receipts (5 runs, full development arc):** `OnlineLoRAStepper` exercised on real LFM2.5-VL — 5/30/50-step stability runs all clean (no divergence, parse_rate 1.000 throughout, OOM ceiling cleared via allocator hygiene). **Class-targeted v2 with action-token-weighted CE: skip class lifted +37.5 pp (0.375 → 0.750) on the held-out probe in 16 steps**, loss drove from 1.15 → 0.0002 — TTT empirically lifts target-class accuracy per pass on a curated stream, not just preserves it. Full development arc + diagnosis in `LFM_TTT_POC.md`.
