...
- **VLA-layer TTT receipts (6 runs, full development arc):** `OnlineLoRAStepper` exercised on real LFM2.5-VL — 5/30/50-step stability runs all clean (no divergence, parse_rate 1.000 throughout, OOM ceiling cleared via allocator hygiene). **Class-targeted v2 with action-token-weighted CE — TWO-CLASS LIFT:** skip class **0.375 → 0.750 (+37.5 pp)**, defer class **0.125 → 0.875 (+75 pp)** on held-out probes in 16 steps each (avg +56.25 pp), loss drove ~1.0 → ~0 in both runs — TTT empirically lifts target-class accuracy per pass on a curated stream, not just preserves it. Full development arc + diagnosis in `LFM_TTT_POC.md`.
