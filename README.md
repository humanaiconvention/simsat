...
- **LFM2.5-VL-450M v3 LoRA fine-tune (canonical, Liquid Track)**: matched-pair holdout 32 samples (8 per class) — `exact_action_agreement` 0.156 → **0.844** (+68.8 pp), `score_mae` 0.365 → **0.055** (−31.0 pp), per-class accept **1.000** / refine **1.000** / defer 0.625 / skip 0.750. Adapter live at [`HumanAIConvention/simsat-lfm25vl-450m-v3`](https://huggingface.co/HumanAIConvention/simsat-lfm25vl-450m-v3). v4 (+20 imbalanced refine/accept rows) regressed −6.3 pp action; v5 (defer-class-balanced, +56 rows total, BEAST overnight) regressed −15.6 pp action with skip per-class crashing 0.75 → 0.25 (model over-shifted toward defer at skip's expense). **Two consecutive negative results on +data confirm v3 is at the inflection point** — the next lift comes from runtime TTT, not more offline data. Recipe + full v4/v5 negatives: `LFM_FINETUNE_METHODOLOGY.md` §6.5/§6.6/§6.7.
- **VLA-layer TTT receipts (7 runs, full development arc):** `OnlineLoRAStepper` exercised on real LFM2.5-VL — 5/30/50-step stability runs all clean (no divergence, parse_rate 1.000 throughout, OOM ceiling cleared via allocator hygiene). **Class-targeted v2 with action-token-weighted CE — TWO-CLASS LIFT:** skip **0.375 → 0.750 (+37.5 pp)**, defer **0.125 → 0.875 (+75 pp)** on held-out probes in 16 steps each (avg +56.25 pp). **Stratified TTT robustness check (full 32-row holdout pre/post):** balanced 4-per-class operator-feedback stream gives net **+3.1 pp** with NO catastrophic regression — single-class TTT is the upper bound of targeted lift; stratified is the safety floor. Full development arc + diagnosis in `LFM_TTT_POC.md`.
...
**Judges, start here** — pick your track's entry point first:
- 🟢 **Liquid Track:** [`LIQUID_TRACK.md`](./LIQUID_TRACK.md) (LFM2.5-VL fine-tune + TTT receipts + MuZero)
- 🔵 **General AI Track:** [`GENERAL_AI_TRACK.md`](./GENERAL_AI_TRACK.md) (Gemma-4-E2B + model-agnostic scaffold + same shared TTT lane)

Then deeper: [SUBMISSION_TLDR.md](./SUBMISSION_TLDR.md) (1 page, 30 sec) → [SUBMISSION_BRIEF.md](./SUBMISSION_BRIEF.md) (full rubric mapping) → [SUBMISSION_CASEBOOK.md](./SUBMISSION_CASEBOOK.md) (4 pinned cases) → [CHALLENGE_ENTRY.md](./CHALLENGE_ENTRY.md) (long-form thesis) → [SUBMISSION_PACKET.md](./SUBMISSION_PACKET.md) (auto-generated state).
...
| | [`LFM_TTT_POC.md`](./LFM_TTT_POC.md) | VLA-layer TTT (online LoRA updates per encounter) — implementation + 7 receipts |
| | [`TTT_RECEIPTS_INDEX.md`](./TTT_RECEIPTS_INDEX.md) | Navigational index across all 7 published TTT receipts |
