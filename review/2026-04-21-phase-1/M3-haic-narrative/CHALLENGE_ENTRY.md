...
Above both planners sits the **HAIC (Human AI Convention) layer**, which turns each decision into a Merkle-rooted receipt over stimulus, interview turns, viability gates, and PRISM entropy. The short version: the encounter pipeline produces decisions, the HAIC layer produces *provable* decisions.
...
> Treating observation opportunities as structured encounter episodes lets us reduce wasted high-cost materialization while preserving high-value windows through an explicit `refine` path — and every decision, accepted or refined, is cryptographically attributable through HAIC.

This challenge entry is intentionally `no-Mapbox-safe`: the core planner, evaluation loop, ObservationVLA trace pipeline, and HAIC convention flow do not require a Mapbox token.
...
- **HAIC convention layer** producing Merkle-rooted receipts (7 leaf commitments, SHA-256 root) for each stimulus → interview → close cycle, with PRISM entropy and viability-gate telemetry
- **Architecture Explorer** visualizing HAIC v3–v8 alongside **Liquid AI's LFM2-8b and LFM2-8b-a1b**, Gemma 3/4, Llama 3.1, and Ministral — making the model landscape the trust layer operates over legible
...
- HAIC layer: [src/sim/haic/](/D:/SimSat/src/sim/haic/) (bridge, interviewer, PRISM loop, PoG, viability, receipt, stimulus + session stores)

## Convention Layer

HAIC sits on top of the same propagator and imagery stack as the encounter planner, but consumes them differently:

- **Stimulus** (`/haic/stimulus`): bundles Sentinel + optional Mapbox payloads with a stable `content_hash`, so the *thing being judged* is reproducible.
- **Session** (`/haic/session`, `/haic/session/{id}/turn`): captures the interviewer ↔ operator exchange plus keystroke telemetry, so the *judgment process* is reproducible.
- **Close** (`/haic/session/{id}/close`): runs PRISM entropy-delta measurement, viability gates, and PoG (proof-of-genuineness) scoring, so the *judgment outcome* is reproducible.
- **Receipt** (`/haic/session/{id}/receipt`): emits a 64-char SHA-256 Merkle root over seven leaf commitments (stimulus, interview, decision, PRISM, viability, PoG, close-summary), so the *auditable claim* is small enough to publish but large enough to prove.

The dashboard surfaces these as four first-class panels: `HAICPanel`, `InterviewPanel`, `PRISMPanel`, `ReceiptPanel`. The runtime flag is `HAIC_PRISM_MODE` (currently `synthetic` for the submission demo) and the sensitivity knob is `HAIC_EPSILON`.
...
5. Open the **Architecture Explorer** via the header button to walk judges through HAIC v3–v8, LFM2-8b, Gemma 4, Llama 3.1, and Ministral — the model landscape the trust layer operates over.
