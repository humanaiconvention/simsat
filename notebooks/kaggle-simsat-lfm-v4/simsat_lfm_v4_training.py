...
Inputs (Kaggle-attached):
  - Dataset: benhaslam/simsat-lfm-v3 mounted at /kaggle/input/simsat-lfm-v3/
        simsat_lfm_train.jsonl     (185 rows, balanced 4-class — v3 adds 20 adversarial reviews)
        simsat_lfm_holdout.jsonl   (32 rows, 8 per class — same as v1, identical for fair v1/v3/v4 compare)
        simsat_lfm_eval.jsonl      (4 legacy rows — defer/refine only)
        images/*.png

v4 changes vs v3:
  - 185 train rows (+12.1% over v3's 165): 52/34/75/24 a/d/r/s
  ...
  - Refine grew 60 -> 75 (+25%); other classes unchanged.

v3 (parent run) holdout: action 0.844, MAE 0.055, refine 1.00, skip 0.75.
...
if not _found:
    raise FileNotFoundError(
        ...
        "Verify the dataset 'benhaslam/simsat-lfm-v3' is attached to this kernel."
    )
...
OUTPUT_DIR = "/kaggle/working/simsat-lfm25vl-450m-v4"
...
#      HumanAIConvention/simsat-lfm25vl-450m-v4.
...
HF_REPO_ID = "HumanAIConvention/simsat-lfm25vl-450m-v4"
