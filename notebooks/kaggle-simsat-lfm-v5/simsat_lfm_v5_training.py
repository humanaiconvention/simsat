...
Inputs (Kaggle-attached):
  - Dataset: benhaslam/simsat-lfm-v5 mounted at /kaggle/input/simsat-lfm-v5/
        simsat_lfm_train.jsonl     (241 rows, balanced 4-class — v5 adds 56 defer-targeted reviews)
        simsat_lfm_holdout.jsonl   (32 rows, 8 per class — same as v1, identical for fair v1/v3/v4/v5 compare)
        simsat_lfm_eval.jsonl      (4 legacy rows — defer/refine only)
        images/*.png

v5 changes vs v3:
  - 241 train rows (+46.1% over v3's 165): 60/51/105/25 a/d/r/s
  - Same recipe as v3 (lr=2e-4, 5 epochs, r=16, alpha=32, effective batch 8,
    assistant-only loss masking, repetition_penalty=1.05 in eval).
  - **Defer grew 34 -> 51 (+50%)**, the class-balanced augmentation v4 was
    missing. v4 (185 train, +20 refine/accept only) regressed defer 0.625 ->
    0.500 because the new data was class-imbalanced. v5 explicitly targets
    the underrepresented classes.

v3 (parent run) holdout: action 0.844, MAE 0.055, defer 0.625, skip 0.75.
v4 holdout: action 0.781, MAE 0.066, defer 0.500, skip 0.625 (regression).
...
get_ipython().system(  # noqa: F821
    "pip install -q --force-reinstall --no-deps 'huggingface_hub==1.14.0' 2>&1 | tail -5"
)
...
if not _found:
    raise FileNotFoundError(
        ...
        "Verify the dataset 'benhaslam/simsat-lfm-v5' is attached to this kernel."
    )
...
def generate_assessment(model, processor, messages_no_assistant: list[dict],
    ...
    v3 decode hardening: applies symmetrically to BASE and TUNED so the
    matched-pair stays fair. Locally validated on v1 adapter (Run A): lifted
    parse_rate 0.906 -> 1.000, exact_action 0.656 -> 0.750, useful_agreement
    0.500 -> 0.844 vs no rep_penalty.
    """
...
OUTPUT_DIR = "/kaggle/working/simsat-lfm25vl-450m-v5"
...
#      HumanAIConvention/simsat-lfm25vl-450m-v5.
...
HF_REPO_ID = "HumanAIConvention/simsat-lfm25vl-450m-v5"
