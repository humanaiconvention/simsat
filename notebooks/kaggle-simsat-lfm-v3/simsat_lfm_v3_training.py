...
Inputs (Kaggle-attached):
  - Dataset: benhaslam/simsat-lfm-v2 mounted at /kaggle/input/simsat-lfm-v2/
        simsat_lfm_train.jsonl     (172 rows, balanced 4-class — v2 adds 51 new ops reviews)
        simsat_lfm_holdout.jsonl   (32 rows, 8 per class — same as v1, identical for fair v1-vs-v3 compare)
        simsat_lfm_eval.jsonl      (4 legacy rows — defer/refine only)
        images/*.png

v3 changes vs v1:
  - 172 train rows (+57.8% over v1's 109): 47/34/60/31 a/d/r/s
  - Decode hardening: repetition_penalty=1.05, no_repeat_ngram_size=20 in
    base AND tuned generation (matched-pair stays fair). Run A locally
    confirmed this single knob lifted v1 holdout +9.4 pp action / +34.4 pp
    useful / parse 0.906 -> 1.000.
...
if not _found:
    raise FileNotFoundError(
        ...
        "Verify the dataset 'benhaslam/simsat-lfm-v2' is attached to this kernel."
    )
...
OUTPUT_DIR = "/kaggle/working/simsat-lfm25vl-450m-v3"
...
HF_REPO_ID = "HumanAIConvention/simsat-lfm25vl-450m-v3"
