#!/usr/bin/env python3
"""
SimSat Gemma-4 v1 Kaggle push script.

Steps:
  1. Run prepare_dataset.py to merge JSONL files into dataset dir
  2. kaggle datasets push  (creates/updates benhaslam/simsat-gemma4-v1)
...
DATASET_DIR = HERE.parent.parent / "datasets" / "simsat-gemma4-v1"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Push SimSat dataset + kernel to Kaggle")
    ...
    if push_dataset:
        ...
        print("=" * 60)
        # Try `version` first (updates existing dataset); fall back to `create` on first push.
        rc = run(["kaggle", "datasets", "version", "-p", str(DATASET_DIR), "-m", "simsat-gemma4-v1 update"], dry_run=args.dry_run)
        if rc != 0:
        ...
        if rc != 0:
            print("Dataset push failed. Check kaggle CLI credentials and that dataset-metadata.json is correct.")
            sys.exit(rc)
        print("Dataset pushed: benhaslam/simsat-gemma4-v1")
    ...
    if push_kernel:
        ...
        if rc != 0:
            ...
            sys.exit(rc)
        print("Kernel pushed: benhaslam/simsat-gemma4-v1-training")

    print("\n" + "=" * 60)
    print("DONE")
    print("=" * 60)
    if not args.dry_run:
        print("Monitor at: https://www.kaggle.com/code/benhaslam/simsat-gemma4-v1-training")
        print("Dataset at: https://www.kaggle.com/datasets/benhaslam/simsat-gemma4-v1")
