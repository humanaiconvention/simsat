...
Gates 1–3 apply at observation intake: quality and metadata checks happen before any adaptation signal is generated. Gates 4–6 apply to the adaptation response itself and are implemented in `evaluate_ttt_viability()` (`src/sim/haic/viability.py`): update magnitude (weight_drift ≤ 0.30 from pre-pass baseline), update rate (cumulative count ceiling), and error balance (`error_bias`: <70% same-sign errors across the last 10 updates). **`error_bias` is the only blocking gate** — it is evaluated *pre-update* in `_apply_trust_layer_ttt()`, so a biased update step is skipped entirely rather than logged after the fact. `weight_drift` and `update_rate` are post-update log-only warnings; they flag but do not block.
...
|---|---|---|
| **Trust-layer TTT** | **Proven in flight (offline replay)** | 10-seed stability: **22.5% MAE improvement ± 0.1%**, 24.1% reward improvement, 1.9 ± 0.3 cycles to 90% convergence. **100-cycle long-horizon run** (25,600 updates): no divergence — post-convergence MAE range ±0.73% of mean across cycles 5–100. **Distribution-shift demo (coastal → polar):** with coastal-optimized weights, polar-corpus MAE = 0.161; after **one cycle of polar feedback**, polar MAE drops to 0.150 (−6.9%); 94% of polar-regime improvement in a single pass. Weight re-adaptation: geometry +0.061, clarity −0.109. This is the on-orbit improvement mechanism — not monotonic improvement on fixed data, but rapid re-convergence per distribution shift. The Rotterdam `accept→refine` loop (`urban_coastal_ambiguity`, trace `4f65355f…`) illustrates the same per-encounter feedback mechanism at case scale. See [`ttt_stability_analysis.md`](./ttt_stability_analysis.md). |
| **Six viability gates** | **Implemented, exercised under structured load** | `evaluate_ttt_viability()` in `src/sim/haic/viability.py`, wired via `_apply_trust_layer_ttt()` in `service.py`. Beyond the anecdotal fires during the 2026-05-06 operator-review session, a structured exercise (1100 updates × 3 synthetic streams — `baseline_clean`, `drift_one_class`, `saturation`) tallies fire rates per gate per condition. Headline: gates are **selective** — `error_bias` **blocks** 99.1% of updates under sustained bias (`drift_one_class`), triggering a **blocking cascade**: weight drift never fires because blocking intercepts before weights can shift. `update_rate` fires only past the 1000-update ceiling on saturated streams. Benign streams stay mostly quiet (error_bias 38.5% is expected statistical behavior for 50/50 errors). See [`VIABILITY_GATES_EXERCISE.md`](./VIABILITY_GATES_EXERCISE.md). |
| **VLA-layer TTT** | **Implemented + unit-tested; live-stream measurement requires prize hardware** | `src/sim/observation_vla/lfm_ttt.py:OnlineLoRAStepper` performs forward + backward + optimizer step on PEFT LoRA weights of a vision-language model, gated by the **same six-gate viability filter** (BLOCKING `error_bias` evaluated pre-update; window advances on block via the `record_skipped_observation` pattern; `weight_drift` on LoRA-delta L2 vs initial-zero baseline). 16 passing unit tests in `tests/test_lfm_ttt.py` prove the gate/step/skip/clear behavior without needing a real LFM checkpoint loaded. The 75-episode MuZero offline-replay eval and the 152-case ObservationVLA eval are still on **static** checkpoints; the live-stream comparison (per-encounter TTT vs static fine-tune) is the experiment the prize hardware (NVIDIA Orin 16 GB + ground-compute days) is allocated to run. See [`LFM_TTT_POC.md`](./LFM_TTT_POC.md). |

The ObservationVLA fine-tunes — v11 (canonical for the General AI track) and v12 (retrain on dataset v4 — completed, not promoted; see line 96 below) — are **static checkpoints that set where drift starts**, not drift-adaptive models in their own right. The Liquid Track has its own LFM2.5-VL fine-tune track (v1 → v3 canonical → v4 negative result → v5 in flight, see line 92). The 56-point gap between v11's in-distribution N=37 (0.86) and cross-distribution N=152 (0.30) eval frames is **what TTT is meant to close per-pass on a live stream**, not what offline retraining closes. v12's contribution is to move the static training distribution closer to deployment so the live TTT loop covers less ground per pass — a complement to the runtime mechanism, not a replacement.
...
- **General AI Track (Gemma-4)** — ObservationVLA backed by a Gemma-4-E2B fine-tune scoped specifically to SimSat's encounter-triage task (accept / defer / skip / refine over Sentinel-style tiles). Training code and dataset at `notebooks/kaggle-simsat-gemma4-v1/`; Kaggle kernel `benhaslam/simsat-gemma4-v1-training` (**v11** = first run with corrected target_modules — see [`GEMMA4_LORA_NULL_TRAINING_AUDIT.md`](./notebooks/GEMMA4_LORA_NULL_TRAINING_AUDIT.md) for why v1-v10 trained nothing); v11 trained on dataset v2 (713 ChatML rows, accept+refine dominant); v12 retrain completed on dataset v4 (1638 rows, balanced 4-class, includes 152 fresh operator-reviewed labels from gallery_review.html) — **eval complete, not promoted, v11 remains canonical** (see line 96 below). Two complementary eval frames:
  ...
  - **v12 (dataset v4 retrain — negative result)**: eval complete; not promoted. 65% parse failure rate (53/152 parsed); effective exact ≈0.18 on the full pool — a regression from v11's 0.30. Parse failures show truncated JSON (`rationale_tags` cut off mid-token). This is the second concrete instance of the labeling-distribution-shift problem alongside v17–v19: dataset manipulation without operator-loop validation broke the operational output contract before model accuracy became measurable. v11 remains canonical. See [`OBSERVATION_VLA_EVAL.md`](./OBSERVATION_VLA_EVAL.md) and KNOWN_ISSUES.md #28.
...
- **Liquid Track:** SimSat couples a vision-language tile encoder (LFM2.5-VL-450M since 2026-04-27, SigLIP-base available as an offline-safe fallback — same one-line slot) with a MuZero planner — the hybrid fits a 5 MB satellite uplink budget while two-scope test-time training adapts both the encoder and the planner in-pass without ground-contact.
...
- The `accept→refine` thesis is demonstrated on the Port of Rotterdam pinned case (`urban_coastal_ambiguity`, `trace_4f65355f5c954fbf8db3fc684bb377af`) — scaffold `accept`, trust `refine`, operator confirmed `accept` — the visible 51% of the port basin was operationally sufficient despite 48.78% cloud cover.
