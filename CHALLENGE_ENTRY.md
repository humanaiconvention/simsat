...
- v11 LoRA partial-save bug (Issue #27): GQA `num_key_value_heads=1` causes PEFT to deduplicate k/v tensors by identity, dropping layers 15–34 k/v LoRA. Fix in v18 (2026-05-05): `data_ptr()` scan on `lora_A`/`lora_B` weights detects tensor-storage sharing regardless of Python object identity; deepcopies only adapter weights (not `bnb.Linear4bit` base_layer); v17 id()-based walk retained as fallback; strict 490-tensor sanity gate. In-repo; Kaggle push pending.
