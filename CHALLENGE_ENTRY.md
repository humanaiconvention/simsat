...
| **Six viability gates** | **Implemented, exercised under structured load** | `evaluate_ttt_viability()` in `src/sim/haic/viability.py`, wired via `_apply_trust_layer_ttt()` in `service.py`. Beyond the anecdotal fires during the 2026-05-06 operator-review session, a structured exercise (1100 updates × 3 synthetic streams — `baseline_clean`, `drift_one_class`, `saturation`) tallies fire rates per gate per condition. Headline: gates are **selective** — `error_bias` fires on 99.8% of steps under sustained operator-vs-prediction bias; `update_rate` fires only past the 1000-update ceiling on saturated streams; benign streams stay mostly quiet. See [`VIABILITY_GATES_EXERCISE.md`](./VIABILITY_GATES_EXERCISE.md). |
