...
- Defer class: 3 original traces → **13** after 10 auto-labeled defer outcomes added 2026-05-05 (cloud_cover≥80% AND target_visible heuristic). Stage 2 v2 predicted 0/75 defer; Stage 2 v3 (run via `bash scripts/muzero_run_stage2_v3.sh`) is the first run where defer reaches target-per-class (48 effective examples, on par with accept/refine). v3 results pending.
