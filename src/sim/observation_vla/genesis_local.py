...
Wraps TransformersVLMAdapter and points it at the Genesis model.
Enable with: OBSERVATION_VLA_BACKEND=genesis
...
Step 1 — Point to your model weights:
    ...
    GENESIS_MODE=merged
    GENESIS_MERGED_PATH=your-hf-username/genesis-152m    # or local /path/to/merged/

    # Option B: Base + LoRA adapter
    GENESIS_MODE=lora
    GENESIS_BASE_MODEL=your-hf-username/genesis-152m-base
    GENESIS_LORA_PATH=/path/to/genesis-simsat-adapter/

    # Option C: Quantized GGUF (best for Orin on-orbit demo)
    GENESIS_MODE=gguf
    GENESIS_GGUF_PATH=/path/to/genesis.gguf
...

{
  ...
  "rationale_tags": ["genesis_local", "sentinel_support"]   // list of strings
}
...
and gate 6 (epistemic alignment). The TTT loop will tune your model's
confidence blend weights over successive passes via online_update() calls.
...
def _genesis_weights_root() -> Path:
    return Path(os.environ.get("GENESIS_WEIGHTS_DIR", "./weights/genesis")).expanduser()


class GenesisAdapter:
    """Genesis model adapter for ObservationVLA.

    Delegates to TransformersVLMAdapter with Genesis-specific env vars and
    patches the returned payload to carry genesis_local branding in
    ...
    def __init__(
        ...
        root = _genesis_weights_root()
        resolved_mode = (mode or os.environ.get("GENESIS_MODE") or "lora").strip().lower()
        resolved_base = base_model or os.environ.get("GENESIS_BASE_MODEL") or ""
        resolved_lora = lora_path or os.environ.get("GENESIS_LORA_PATH") or str(root / "adapter")
        resolved_merged = merged_path or os.environ.get("GENESIS_MERGED_PATH") or str(root / "base")
        resolved_gguf = gguf_path or os.environ.get("GENESIS_GGUF_PATH") or str(root / "gguf" / "genesis.gguf")
        resolved_device = device or os.environ.get("OBSERVATION_VLA_DEVICE") or "cpu"
        resolved_label = model_label or os.environ.get("GENESIS_MODEL_LABEL") or "genesis"
    ...
    def model_id(self) -> str:
        ...
        # "transformers_vlm:{mode}:{label}" → "genesis:{mode}:{label}"
        return delegate_id.replace("transformers_vlm:", "genesis:", 1)
    ...
    def assess(
        ...
        # Patch branding so the service layer sees genesis_local identity.
        payload["_model_id"] = self.model_id
        tags: list[str] = list(payload.get("rationale_tags", []))
        if "genesis_local" not in tags:
            tags = ["genesis_local"] + [t for t in tags if t != "transformers_vlm_local"]
            payload["rationale_tags"] = list(dict.fromkeys(tags))
        raw = payload.get("raw_response_text", "")
        if raw.startswith("transformers_vlm_local:"):
            payload["raw_response_text"] = "genesis_local:" + raw[len("transformers_vlm_local:"):]
        return payload
