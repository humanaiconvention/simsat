...
| 25 | Kaggle Gemma-4-E2B v2 kernel — gradient flow | `notebooks/kaggle-simsat-gemma4-v1/` | **In progress (v8 RUNNING 2026-04-26, session 7 autonomous)** — v3 errored on TRL 0.15 import; **Fix #15 (v4)** pinned trl<0.15.0; v4 errored with `KeyError: 'text'`; **Fix #16 (v5)** pre-formatted via apply_chat_template; v5/v6 errored with `ValueError: excessive nesting`; **Fix #17 (v6)** added `max_seq_length=1024` and `padding_side="right"`; **Fix #18 (v7)** pre-tokenizes the dataset (custom collator expects input_ids, not text); v7 trained successfully and produced an adapter, but eval showed 7/7 JSON parse failures — model emitted unstructured text. Manual masking diagnostic on BEAST revealed that `response_template="<start_of_turn>model\n"` tokenizes to 9 word-piece tokens that NEVER appear in the chat-templated stream; Gemma-4 uses special token `<\|turn>` (id 105). DataCollatorForCompletionOnlyLM never found the marker so masking was effectively broken — the model trained on the entire conversation structure rather than just the assistant turn. **Fix #19 (v8)**: change response_template to `<\|turn>model\n` (encodes to [105, 4368, 107] which DOES match). Pushed; RUNNING. Diagnostic script also rewritten (no longer depends on TRL's removed-in-0.15 collator class). |
