...
| 6 | ObservationVLA usefulness-score MAE 0.27 — model under-confident vs. operator | `OBSERVATION_VLA_EVAL.md` | **Improved with v9 (session 7 autonomous)** — switched from clip_local (MAE=0.27) to gemma4-simsat fine-tune (MAE=**0.16**, n=7). 3/3 ground-truth `accept` cases identified within 0.05. The remaining miss is a known training-data weight imbalance (model picks `accept` too often on borderline `refine` cases like Rotterdam/Fort Myers) — flagged for future v10 with re-balanced epoch weights. |
...
| 25 | Kaggle Gemma-4-E2B v2 kernel — gradient flow | `notebooks/kaggle-simsat-gemma4-v1/` | **Resolved (v9 + adapter fix, 2026-04-26 session 7 autonomous)** — Iterative fix-train: TRL import (#15) → KeyError text (#16) → ValueError nesting (#17, #18) → wrong response_template (#19) → trailing-comma syntax (#20). v9 trained successfully and BEAST diagnostic confirmed masking now healthy (74.67% mask ratio, 0 unmatched). Eval STILL showed JSON parse failures — but root cause was an ADAPTER bug, not the model: `_generate_text` and `_generate_vlm` in `transformers_vlm_local.py` sliced output by `full_text.startswith(chat_prompt)` after `decode(skip_special_tokens=True)` — strips `<\|turn>` from output but not from chat_prompt, startswith fails, returns prompt+response. **Fix #21 (adapter)**: slice on token IDs (`out[0, prompt_len:]`) before decoding. Re-run eval shows: bucketed action agreement 0.57 (was 0.43 fallback), usefulness MAE 0.16 (was 0.27 clip_local baseline / 0.21 fallback), zero parse failures, 3/3 ground-truth `accept` cases identified within 0.05 of operator score (Suez 0.94 vs 0.95, Houston 0.88 vs 0.92, SF Bay 0.87 vs 0.90). General-AI track has a real working backend. |
| 26 | Genesis + Tesseract T3 model weights | `src/sim/observation_vla/genesis_local.py`, `tesseract_t3_local.py` | **Genesis adapter live (2026-04-26, session 8)** — `genesis_local.py` rewritten as native loader using orchOSModel-genesis-v3 directly (no HF dependency). Loads `genesis_152m_instruct.safetensors` via safetensors metadata config + NeoX tokenizer + ChatML format. Model loads cleanly on BEAST (151.76M params, GLA+FoX hybrid). Zero-shot JSON output unreliable at 152M — Guilherme needs to fine-tune on simsat-gemma4-v1 dataset (same as Gemma-4 track) or provide a working system prompt. Setup: `GENESIS_REPO_PATH=/path/to/orchOSModel-genesis-v3 GENESIS_WEIGHTS_PATH=/path/to/genesis_152m_instruct.safetensors OBSERVATION_VLA_BACKEND=genesis`. Tesseract T3 still pending Garrett. |
