...
| 27 | v11 adapter partial save — k_proj/v_proj LoRA missing for layers 15-34 | `notebooks/kaggle-simsat-gemma4-v1/simsat_gemma4_v1_training.py` save path | **Fix in v12 (local commit ea56234, push pending)** — v11 saves only 410 of 490 expected LoRA tensors. Root cause: Gemma-4-E2B GQA (`num_key_value_heads=1`) causes `tie_weights()` to alias k/v tensors across layer groups; PEFT `state_dict` deduplicates by tensor identity, dropping layers 15-34 k/v LoRA. v12 fix: `peft>=0.20.0` + explicit `weight.data.clone()` on all LoRA params before `save_pretrained` + strict 490-tensor sanity gate (35 layers × 7 modules × 2). Push `fork collab/heatmap-backend` and run as Kaggle kernel v12 to confirm `LoRA tensor coverage: 490/490 (PASS)`. |
