...
| 27 | v11 adapter partial save — k_proj/v_proj LoRA missing for layers 15-34 | `notebooks/kaggle-simsat-gemma4-v1/simsat_gemma4_v1_training.py` save path | **Fixed in v18 (2026-05-05)** — Root cause: Gemma-4-E2B GQA shares `k_proj`/`v_proj` `nn.Linear` module *objects*; PyTorch `named_parameters()` deduplicates by module id, silently dropping layers 15-34 k/v LoRA (410/490 tensors). v12/v13 `.weight.data.clone()` insufficient — dedup is at module-object level. v14-v16: `deepcopy` any repeated `lora_A`/`lora_B` `ModuleDict` entry, but detection used `named_modules()` which itself deduplicates — fix never fired. v17 (`_break_lora_sharing`): walks `_modules` directly (bypasses dedup), tracks module identity via `id(child)` — correct approach but only catches shared module *objects*, not shared tensor *data* (same data in distinct wrapper objects). Also had a scoping bug (`global _fixed_v17` in nested function, should be `nonlocal`). **v18 (committed 2026-05-05)**: `_break_lora_sharing` replaces module-id check with `tensor.data_ptr()` scan — iterates `child.lora_A.values()` + `child.lora_B.values()`, checks `linear.weight.data_ptr()` against a seen-set; on any hit, deepcopies only `lora_A`/`lora_B` (small fp16 matrices), NOT `base_layer` (bnb.Linear4bit, not reliably deepcopy-safe). v17 id()-based walk retained as fallback if `_n_fixed==0`. Also fixes: `nonlocal _fixed_v17` in fallback, OUTPUT_DIR → `simsat-gemma4-v18-adapter`, version string → `simsat-gemma4-v18`. Needs Kaggle push and run to confirm 490/490. **v17 Kaggle run (2026-05-06)** confirmed: 158/158 steps, QLoRA NF4 r=64, adapter uploaded to `HumanAIConvention/simsat-gemma4-v17` (387 MB adapter_model.safetensors). v17 upload hit a secondary bug: SFTTrainer writes local Kaggle path (`/kaggle/input/models/google/gemma-4/.../1`) into README.md `base_model` field; HF upload_folder rejects it as invalid metadata. Fixed in v18 notebook: README.md base_model path patched to `google/gemma-4-e2b-it` before `upload_folder`. HF upload cell added to end of notebook targeting `HumanAIConvention/simsat-gemma4-v18`. Ready for Kaggle run. |
