...
| 25 | Kaggle Gemma-4-E2B v2 kernel — gradient flow | `notebooks/kaggle-simsat-gemma4-v1/` | **In progress (v6 RUNNING 2026-04-25, session 6)** — v3 errored on TRL 0.15 import; **Fix #15 (v4)** pinned trl<0.15.0; v4 errored with `KeyError: 'text'` at SFTTrainer line 416 — root cause: SFTTrainer's default `dataset_text_field='text'` doesn't match the dataset's `messages` column. **Fix #16 (v5)**: pre-format via `tokenizer.apply_chat_template(...)` before SFTTrainer. v5 got past Fix #16 (model loaded, LoRA wrapped at 0.44% trainable, tokenizer set) but errored with `ValueError: Unable to create tensor` because Gemma's `tokenizer.model_max_length` is effectively infinity → SFTTrainer didn't truncate → ragged batch → collator tensor conversion failed. **Fix #17 (v6)**: explicit `max_seq_length=1024` in SFTConfig + `tokenizer.padding_side="right"`. v6 pushed; status RUNNING; expected ~35–50 min training + ~5 min eval. Triage tool ready: `python scripts/diagnose_gemma4_checkpoint.py --check both --adapter-path ./weights/gemma4-v6/adapter`. |
