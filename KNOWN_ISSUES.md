...
| 25 | Kaggle Gemma-4-E2B v2 kernel — gradient flow | `notebooks/kaggle-simsat-gemma4-v1/` | **In progress (v7 RUNNING 2026-04-25, session 7 autonomous)** — v3 errored on TRL 0.15 import; **Fix #15 (v4)** pinned trl<0.15.0; v4 errored with `KeyError: 'text'`; **Fix #16 (v5)** pre-formatted via apply_chat_template; v5/v6 errored with `ValueError: excessive nesting`; **Fix #17 (v6)** added `max_seq_length=1024` and `padding_side="right"` (necessary but not sufficient); **Fix #18 (v7)**: SFTTrainer skips its auto-tokenization when a custom data_collator is provided. DataCollatorForCompletionOnlyLM expects pre-tokenized `input_ids` (not raw text) — the "excessive nesting" error was the collator getting `{"text": "<string>"}` instead of `{"input_ids": [int, int, ...]}`. v7 tokenizes in the same `.map()` pass as the chat template. Pushed; status RUNNING. |
