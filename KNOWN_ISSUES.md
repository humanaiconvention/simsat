...
| 27 | v11 adapter partial save — k_proj/v_proj LoRA missing for layers 15-34 | `notebooks/kaggle-simsat-gemma4-v1/simsat_gemma4_v1_training.py` save path | **Fix in v14 (running 2026-05-05)** — Root cause (confirmed): Gemma-4-E2B GQA shares `k_proj`/`v_proj` `nn.Linear` module *objects* across layer groups; PyTorch `named_parameters()` deduplicates by module id before PEFT builds the state dict, silently dropping layers 15-34 k/v LoRA (410/490 tensors). v12/v13 `.weight.data.clone()` fix was insufficient — dedup happens at module-object level, not tensor level. v14 fix: `deepcopy` any repeated `lora_A`/`lora_B` `ModuleDict` entry before `save_pretrained` so every path gets a distinct module id (commit 01813fc). Kaggle kernel v14 running; expected `LoRA tensor coverage: 490/490 (PASS)`. |
