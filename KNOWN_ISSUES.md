...
| 25 | Kaggle Gemma-4-E2B v2 kernel — gradient flow | `notebooks/kaggle-simsat-gemma4-v1/` | **In progress (v5 RUNNING 2026-04-25, session 6)** — v3 errored on TRL 0.15 import; **Fix #15 (v4)** pinned trl<0.15.0; v4 errored with `KeyError: 'text'` at SFTTrainer line 416 — root cause: SFTTrainer's default `dataset_text_field='text'` doesn't match the dataset's `messages` (ChatML) column. **Fix #16 (v5)**: pre-format the dataset via `tokenizer.apply_chat_template(...)` before SFTTrainer, producing a `text` column the collator can mask. v5 pushed via `kaggle kernels push`; status RUNNING; expected ~35–50 min training + ~5 min eval. Triage tool ready: `python scripts/diagnose_gemma4_checkpoint.py --check both --adapter-path ./weights/gemma4-v5/adapter`. |
