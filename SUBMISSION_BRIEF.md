...
- **LFM2-VL fine-tuning** (rubric: "strongly encouraged and rewarded with documented methodology, measurable improvement over the base model, and publicly shared weights and training code"):
    ...
    - **v4 negative result (kept v3 canonical):** v4 trained on +20 more operator-reviewed rows (185 train) and regressed -6.3 pp action / -12.5 pp on defer / -12.5 pp on skip. Diagnosis: the +20 reviews were 16 refine + 4 accept (no defer/skip), pushing the model in a direction that hurt minority-class accuracy. v4 weights remain public on Kaggle for transparency; not promoted to HuggingFace. v3 stays canonical. Detail in [`LFM_FINETUNE_METHODOLOGY.md`](./LFM_FINETUNE_METHODOLOGY.md) §6.6.
