...
- **LFM2-VL fine-tuning** (rubric: "strongly encouraged and rewarded with documented methodology, measurable improvement over the base model, and publicly shared weights and training code"):
    ...
    - **Measurable improvement over base:** matched-pair base-vs-tuned eval on the 32-row stratified hold-out (8 per class × 4 classes). Numbers update here once the Kaggle kernel completes.
    - **Publicly shared weights:** the LoRA adapter ships to `HumanAIConvention/simsat-lfm25vl-450m-v1` on HF (Cell 11 of the kernel, gated until eval metrics are reviewed).
