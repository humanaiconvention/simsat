...
| **VLA-layer TTT** — VLM weights / LoRA adapters adapt on streaming Sentinel tiles per-pass, gated by all six checks | **Implemented + unit-tested; live-stream measurement requires prize hardware** | `OnlineLoRAStepper` in `src/sim/observation_vla/lfm_ttt.py` performs per-encounter LoRA gradient updates gated by the **same six-gate viability filter** as the trust layer (BLOCKING `error_bias` evaluated pre-update; window advances on block; `weight_drift` on LoRA-delta L2). 16 passing unit tests prove gate/step/skip/clear behavior. Live-stream measurement of TTT vs static fine-tune is the experiment the prize hardware enables — see [`LFM_TTT_POC.md`](./LFM_TTT_POC.md). |
...
- **LFM2-VL fine-tuning** (rubric: "strongly encouraged and rewarded with documented methodology, measurable improvement over the base model, and publicly shared weights and training code"):
    - **Documented methodology:** [`LFM_FINETUNE_METHODOLOGY.md`](./LFM_FINETUNE_METHODOLOGY.md) — covers target_modules selection (verbatim from `Liquid4All/leap-finetune` `DEFAULT_VLM_LORA`), stratified 8-per-class hold-out construction, three concentric sanity gates, matched-pair eval protocol, and disclosed limitations.
    - **Training code:** [`notebooks/kaggle-simsat-lfm-v3/`](./notebooks/kaggle-simsat-lfm-v3/) — Kaggle T4 kernel for the canonical v3 adapter (runs base eval + LoRA training + tuned eval + side-by-side metrics in one Run All; 23 invariant checks before pushing). The v1 scaffold lives at [`notebooks/kaggle-simsat-lfm-v1/`](./notebooks/kaggle-simsat-lfm-v1/) (with [`HANDOFF.md`](./notebooks/kaggle-simsat-lfm-v1/HANDOFF.md) covering the recovery-paths story for the v1 environment cascade), and the v4 negative-result + v5 (defer-class-balanced retrain on BEAST) sit at [`notebooks/kaggle-simsat-lfm-v4/`](./notebooks/kaggle-simsat-lfm-v4/) and [`notebooks/kaggle-simsat-lfm-v5/`](./notebooks/kaggle-simsat-lfm-v5/) respectively.
    - **Measurable improvement over base — v3 canonical adapter:** matched-pair base-vs-tuned eval on the 32-row stratified hold-out (8 per class × 4 classes), held identical between v1 and v3 for direct comparison. **v3 delivered:** `exact_action_agreement` 0.156 → **0.844** (+68.8 pp), `score_mae` 0.365 → **0.055** (-31.0 pp, lower is better), `useful_agreement` 0.312 → 0.688 (+37.5 pp), `parse_rate` 1.000 → **1.000**. Per-class action accuracy (tuned): accept **1.000** / refine **1.000** / defer 0.625 / skip 0.750. The +18.8 pp action lift over v1's first run came from two compounding levers: (1) decode hardening at eval time (`repetition_penalty=1.05`, `no_repeat_ngram_size=20`) — Run A confirmed this single knob lifts v1 +9.4 pp action and parse_rate to 1.000; (2) +56 operator-reviewed train rows from two gallery review sessions on materialized encounters (109 → 165 train; refine 27 → 60 → per-class 0.625 → 1.000).
    - **Publicly shared weights:** v3 LoRA adapter live at [`HumanAIConvention/simsat-lfm25vl-450m-v3`](https://huggingface.co/HumanAIConvention/simsat-lfm25vl-450m-v3) (Apache-2.0, 17.9 MB safetensors, README documents recipe + holdout numbers). v1 retained at [`HumanAIConvention/simsat-lfm25vl-450m-v1`](https://huggingface.co/HumanAIConvention/simsat-lfm25vl-450m-v1) for reference (marked superseded). Training kernels public: `benhaslam/simsat-lfm2-5-vl-v1-training`, `benhaslam/simsat-lfm2-5-vl-v3-training`, and `benhaslam/simsat-lfm2-5-vl-v4-training` on Kaggle.
    - **Three consecutive negative attempts to beat v3 (kept v3 canonical):** v4 (+20 imbalanced refine/accept rows, 185 train) regressed -6.3 pp action. v5 (defer-class-balanced augmentation, +56 rows, 241 train) regressed -15.6 pp action with skip per-class crashing 0.750 → 0.250. v3+ (recipe variant — `lr=1e-4` vs `2e-4`, 8 epochs vs 5, `lora_dropout=0.10` vs 0.05, **same 165-row v2 dataset as v3** to isolate recipe from data) regressed -3.1 pp action; same useful-vs-exact trade-off as v5. **Three independent lines of evidence — class-imbalanced data, class-balanced data, recipe variant — all converge on the same conclusion: v3 sits at a local optimum on this architecture × data × holdout combination that further offline tuning cannot escape.** This is empirical evidence (not just architectural assertion) for the runtime-TTT lane being the next investment. v4/v5/v3+ weights kept for transparency; only v3 promoted to HuggingFace. Full diagnostics in [`LFM_FINETUNE_METHODOLOGY.md`](./LFM_FINETUNE_METHODOLOGY.md) §6.6 / §6.7 / §6.8.
    - **TTT receipts — seven runs documenting a complete development arc** (closes prior gaps in [`LFM_TTT_POC.md`](./LFM_TTT_POC.md)):
        - **(1) Proof-of-life (5 steps, v1 adapter):** mechanism works — 5/5 applied, 0 gates triggered. Step 6 OOM'd at 8 GB ceiling.
        - **(2) Extended stability v1 (30 steps, v3 adapter, 8-row probe):** 28/30 applied, 0 OOM (allocator hygiene cleared the ceiling), parse_rate 1.000 throughout, `lora_delta_l2` monotonic, steady-state by step 10.
        - **(3) Extended stability v2 (50 steps, v3 adapter, 16-row probe):** 48/50 applied, 0 OOM, same steady-state pattern with tighter probe. Trust-layer TTT has 100-cycle evidence; VLA-layer now has 50-cycle.
        - **(4) Class-targeted v1, full-CE loss (negative result, kept for transparency):** skip class regressed 0.375 → 0.000. Diagnosis: full-assistant CE diluted the action-token gradient signal across ~99 non-action tokens. The model learned the JSON template, not the action.
        - **(5) Class-targeted v2, action-token-weighted CE — HEADLINE RESULTS (two-class lift):** `forward_loss` masks all assistant tokens except the `recommended_action` value range. **Skip class** (16 skip-only train → 8 skip-only holdout probe): **0.375 → 0.750 (+37.5 pp)**, MAE 0.237 → 0.162. **Defer class** (16 defer-only train → 8 defer-only holdout probe): **0.125 → 0.875 (+75 pp)**, MAE 0.131 → 0.056. Loss in both runs drove from ~1.0 → ~0 over 16 steps, `lora_delta_l2` grew monotonically 0.008 → 0.04. **Average lift across the two classes = +56.25 pp on a held-out probe in 16 sequential gradient steps under viability gates.** **This is the empirical evidence that runtime TTT under operator-curated stream + appropriate loss formulation lifts target-class performance per pass — the operational architectural claim, validated end-to-end on TWO independent classes on a real LFM2.5-VL checkpoint.**
        - **(6) Stratified TTT v2 — robustness check (full 32-row holdout pre/post):** balanced operator-feedback stream (4 per class × 4 classes = 16 steps) measured against the **full 32-row v3 holdout pre and post**. Per-class shifts: accept 1.000 → 1.000, defer 0.125 → 0.000 (−0.125), refine 1.000 → 0.875 (−0.125), **skip 0.375 → 0.750 (+0.375)**. Net overall: **exact_action 0.625 → 0.656 (+0.031)**, score_mae 0.092 → 0.089 (−0.003). **No catastrophic regression** under stratified TTT. Architectural lesson: the single-class runs are the *upper bound of what targeted TTT can lift*; stratified TTT is the *baseline of what mixed feedback does*; the curation policy (which class gets the stream's focus) is the top-level engineering decision the prize hardware lane makes per mission.
        - Receipts: [`.kaggle_output/ttt_proof_of_life_receipt.json`](./.kaggle_output/ttt_proof_of_life_receipt.json), [`.kaggle_output/extended_ttt_receipt.json`](./.kaggle_output/extended_ttt_receipt.json), [`.kaggle_output/extended_ttt_v2_receipt.json`](./.kaggle_output/extended_ttt_v2_receipt.json), [`.kaggle_output/class_targeted_ttt_receipt.json`](./.kaggle_output/class_targeted_ttt_receipt.json), [`.kaggle_output/class_targeted_ttt_v2_receipt.json`](./.kaggle_output/class_targeted_ttt_v2_receipt.json), [`.kaggle_output/class_targeted_ttt_v2_defer_receipt.json`](./.kaggle_output/class_targeted_ttt_v2_defer_receipt.json), [`.kaggle_output/stratified_ttt_v2_receipt.json`](./.kaggle_output/stratified_ttt_v2_receipt.json).
    - In parallel, the encoder also feeds the Liquid Track MuZero BC head, fine-tuned on operator-labeled Sentinel tiles via LFM embeddings. 10-seed BC sweep on the post-N=152 corpus: best val_acc **0.898 ± 0.049** [0.825, 0.975].
