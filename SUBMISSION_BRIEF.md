...
- **LFM2-VL fine-tuning** (rubric: "strongly encouraged and rewarded with documented methodology, measurable improvement over the base model, and publicly shared weights and training code"):
    ...
    - **Training code:** [`notebooks/kaggle-simsat-lfm-v1/`](./notebooks/kaggle-simsat-lfm-v1/) — single Kaggle T4 kernel runs base eval + LoRA training + tuned eval + side-by-side metrics in one Run All. 23 invariant checks before pushing. [`HANDOFF.md`](./notebooks/kaggle-simsat-lfm-v1/HANDOFF.md) walks through the recovery paths.
