...
- **LFM2-VL fine-tuning** (rubric: "strongly encouraged and rewarded with documented methodology, measurable improvement over the base model, and publicly shared weights and training code"):
    ...
    - **v4 + v5 negative results (kept v3 canonical):** v4 (+20 imbalanced refine/accept rows, 185 train) regressed -6.3 pp action / -12.5 pp on defer / -12.5 pp on skip. v5 (defer-class-balanced augmentation, +17 defer / +1 skip / 241 train, BEAST overnight) regressed -15.6 pp action with skip per-class crashing 0.750 → 0.250 — the +17 defer rows pulled the borderline boundary toward defer at skip's expense. **Two consecutive negative results on +data confirm v3 is at the inflection point** of the training-data curve for this architecture / holdout — the next lift comes from runtime TTT under viability gates, not more offline data. Both v4 and v5 weights kept for transparency (v4 on Kaggle, v5 local); only v3 promoted to HuggingFace. Full diagnostics in [`LFM_FINETUNE_METHODOLOGY.md`](./LFM_FINETUNE_METHODOLOGY.md) §6.6 / §6.7.
