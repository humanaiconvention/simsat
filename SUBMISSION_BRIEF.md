...
- **LFM2-VL fine-tuning** (rubric: "strongly encouraged and rewarded with documented methodology, measurable improvement over the base model, and publicly shared weights and training code"):
    ...
    - **TTT receipts — five runs documenting a complete development arc** (closes prior gaps in [`LFM_TTT_POC.md`](./LFM_TTT_POC.md)):
        ...
        - **(5) Class-targeted v2, action-token-weighted CE — HEADLINE RESULT:** `forward_loss` masks all assistant tokens except the `recommended_action` value range. With 16 skip-only train rows as the stream and 8 skip-only holdout rows as the probe: **skip class lifted 0.375 → 0.750 (+0.375, +37.5 pp)**, score_mae 0.237 → 0.162 (-0.075), loss trajectory 1.15 → 0.0002 (drove to near-zero), `lora_delta_l2` 0.0081 → 0.0408. **This is the empirical evidence that runtime TTT under operator-curated stream + appropriate loss formulation lifts target-class performance per pass — the operational architectural claim, validated end-to-end on a real LFM2.5-VL checkpoint.** Receipts: [`.kaggle_output/ttt_proof_of_life_receipt.json`](./.kaggle_output/ttt_proof_of_life_receipt.json), [`.kaggle_output/extended_ttt_receipt.json`](./.kaggle_output/extended_ttt_receipt.json), [`.kaggle_output/extended_ttt_v2_receipt.json`](./.kaggle_output/extended_ttt_v2_receipt.json), [`.kaggle_output/class_targeted_ttt_receipt.json`](./.kaggle_output/class_targeted_ttt_receipt.json), [`.kaggle_output/class_targeted_ttt_v2_receipt.json`](./.kaggle_output/class_targeted_ttt_v2_receipt.json).
