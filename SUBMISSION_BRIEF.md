...
- **LFM2-VL fine-tuning** (rubric: "strongly encouraged and rewarded with documented methodology, measurable improvement over the base model, and publicly shared weights and training code"):
    ...
    - **Measurable improvement over base:** matched-pair base-vs-tuned eval on the 32-row stratified hold-out (8 per class × 4 classes). **Run 14 (v2.2) delivered:** `exact_action_agreement` 0.250 → **0.656** (+40.6 pp), `score_mae` 0.312 → **0.102** (-21.1 pp, lower is better), `useful_agreement` 0.500 → 0.500 (flat), `parse_rate` 1.000 → 0.906 (3 of 32 hit a single-scene `0000…` repetition loop, fixable at decode with `repetition_penalty`). Per-class action accuracy (tuned): accept 1.00 / defer 0.625 / refine 0.625 / skip 0.375. The 40.6 pp action lift and 21.1 pp MAE drop are the headline; the parse dip is concentrated and decode-side, not a representational regression.
    - **Publicly shared weights:** LoRA adapter live at [`HumanAIConvention/simsat-lfm25vl-450m-v1`](https://huggingface.co/HumanAIConvention/simsat-lfm25vl-450m-v1) (Apache-2.0, 17.9 MB safetensors, README documents recipe + holdout numbers). Training kernel publicly visible at `benhaslam/simsat-lfm2-5-vl-v1-training` on Kaggle.
